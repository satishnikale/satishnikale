<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,100:1e293b&height=200&section=header&text=Satish%20Nikale&fontSize=48&fontColor=38bdf8&fontAlignY=35&desc=Full%20Stack%20Software%20Engineer&descAlignY=55&descSize=18&animation=fadeIn" />

<br/>

<a href="https://www.linkedin.com/">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=22&duration=3000&pause=1000&color=38BDF8&center=true&vCenter=true&width=600&lines=Building+scalable+backend+systems;REST+APIs+%2B+Real-time+Systems;Currently%3A+Software+Engineer+Intern+%40+EzyEV;Docker+%2F+AWS+%2F+System+Design" alt="Typing SVG" />
</a>

</div>

<br/>

<div align="center">

[![Email](https://img.shields.io/badge/-Email-0f172a?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:youremail@example.com)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0f172a?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://linkedin.com/in/yourhandle)
[![Portfolio](https://img.shields.io/badge/-Portfolio-0f172a?style=for-the-badge&logo=vercel&logoColor=white)](https://yourportfolio.dev)
[![LeetCode](https://img.shields.io/badge/-LeetCode-0f172a?style=for-the-badge&logo=leetcode&logoColor=FFA116)](https://leetcode.com/yourhandle)
[![HackerRank](https://img.shields.io/badge/-HackerRank-0f172a?style=for-the-badge&logo=hackerrank&logoColor=2EC866)](https://hackerrank.com/yourhandle)
[![GFG](https://img.shields.io/badge/-GeeksforGeeks-0f172a?style=for-the-badge&logo=geeksforgeeks&logoColor=2F8D46)](https://geeksforgeeks.org/user/yourhandle)

</div>

<br/>

## About

Full stack engineer based in Pune, India, currently working as a **Software Engineer Intern at EzyEV Technologies**, where I build production backend systems handling payments, KYC verification, and automated billing.

I spend most of my time on backend architecture — designing APIs that hold up under real traffic, not just demos. MCA graduate, self-taught before that.

Outside of work: distributed systems, system design write-ups, and the occasional over-engineered side project.

<br/>

## Tech Stack

<table>
<tr>
<td valign="top" width="50%">

**Frontend**

<img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,html,css,tailwind" />

</td>
<td valign="top" width="50%">

**Backend**

<img src="https://skillicons.dev/icons?i=nodejs,express,postgres,mongodb,mysql,prisma" />

</td>
</tr>
<tr>
<td valign="top" width="50%">

**DevOps & Infra**

<img src="https://skillicons.dev/icons?i=docker,git,githubactions,aws,supabase" />

</td>
<td valign="top" width="50%">

**Languages**

<img src="https://skillicons.dev/icons?i=ts,js,python,java,cpp" />

</td>
</tr>
</table>

<sub>REST APIs · WebSockets · GitHub Actions (learning) · AWS (learning) · Turborepo</sub>

<br/>

## What I Build

<table>
<tr>
<td width="25%" align="center">🔌<br/><b>Scalable REST APIs</b></td>
<td width="25%" align="center">🔐<br/><b>Auth Systems</b></td>
<td width="25%" align="center">💳<br/><b>Payment Integrations</b></td>
<td width="25%" align="center">🪪<br/><b>KYC Pipelines</b></td>
</tr>
<tr>
<td width="25%" align="center">📊<br/><b>Dashboard Apps</b></td>
<td width="25%" align="center">⚡<br/><b>Real-time Systems</b></td>
<td width="25%" align="center">🛠️<br/><b>Internal Dev Tools</b></td>
<td width="25%" align="center">🏗️<br/><b>Backend Architecture</b></td>
</tr>
</table>

<br/>

## Experience

```
2024 — Present    Software Engineer Intern, EzyEV Technologies
                   Production backend for EV platform — payments, KYC, billing

                   MCA — Master of Computer Applications
```

<br/>

## Featured Projects

<table>
<tr>
<td width="33%" valign="top">

### 🎨 Excalidraw Clone

Real-time collaborative whiteboard with canvas sync and session persistence.

`Next.js` `TypeScript` `WebSockets`
`Prisma` `PostgreSQL` `JWT` `Turborepo`

- Live canvas synchronization
- Room-based authentication
- Persistent drawing sessions

</td>
<td width="33%" valign="top">

### 📚 StudyNotion

Full-featured learning management system with instructor tooling and payments.

`React` `Node.js` `Express`
`MongoDB` `Razorpay` `Cloudinary`

- Course creation & delivery
- Integrated payment flow
- Instructor dashboard

</td>
<td width="33%" valign="top">

### ⚡ EzyEV

Production backend powering an EV services platform.

`Payments` `KYC` `Automation`

- Cashfree KYC verification
- Razorpay payment gateway
- WhatsApp automation
- Billing infrastructure

</td>
</tr>
</table>

<br/>

## Currently Working On

- [x] Production backend systems at EzyEV
- [ ] Docker in depth
- [ ] AWS — deployment & infra
- [ ] CI/CD pipelines
- [ ] System design fundamentals
- [ ] Distributed systems

<br/>

## GitHub Analytics

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=yourusername&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=38bdf8&icon_color=38bdf8&text_color=c9d1d9" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=yourusername&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=38bdf8&text_color=c9d1d9" />

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=yourusername&theme=tokyonight&hide_border=true&background=0d1117&stroke=38bdf8&ring=38bdf8&fire=38bdf8&currStreakLabel=38bdf8" />

<br/>

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=yourusername&theme=tokyo-night&hide_border=true&bg_color=0d1117&color=38bdf8&line=38bdf8&point=ffffff" />

</div>

<br/>

## Trophies

<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=yourusername&theme=tokyonight&no-frame=true&no-bg=true&row=1&margin-w=15" />
</div>

<br/>

## Contribution Snake

<div align="center">
<img src="https://raw.githubusercontent.com/yourusername/yourusername/output/github-contribution-grid-snake-dark.svg" />
</div>

<br/>

<div align="center">

### 💭 Quote

<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" />

</div>

<br/>

<div align="center">

<img src="https://komarev.com/ghpvc/?username=yourusername&color=38bdf8&style=flat-square&label=Profile+Views" />

<br/><br/>

<sub>Built with intent, not templates.</sub>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:1e293b,100:0f172a&height=100&section=footer" />

</div>
